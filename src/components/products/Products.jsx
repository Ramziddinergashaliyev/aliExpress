        ...
        <ul className="products__category">
          <li>
            <data
              style={{
                background:
                  this.state.categoryValue === `/products` ? "black" : "red",
                color:
        ...
        <div className="products__cards">
          {this.state.data?.map((el) => (
            <div key={el.id} className="products__card">
              <div className="products__card__imgs">
                <img src={el.images[0]} alt={el.title} />
              </div>
              <h3 className="products__card__title">{el.title}</h3>
              <p className="products__card__price">price: {el.price}USD</p>
